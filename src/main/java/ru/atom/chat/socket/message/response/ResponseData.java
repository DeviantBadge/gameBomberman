...
public class ResponseData extends ArrayList<Mail> {
    ...
    public void addCommonMessage(Message message) {
        OutgoingChatMessage outgoingChatMessage = new OutgoingChatMessage(message);
        Mail mail = buildMail(MailingType.TO_ALL, OutgoingTopic.NEW_MESSAGE, JsonHelper.toJson(outgoingChatMessage));
        add(mail);
    }
